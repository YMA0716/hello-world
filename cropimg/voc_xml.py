...
class PascalVocWriter:
    ...
    writer = PascalVocWriter('VOC2007', img_name, img.shape)
    for r in result:
        label, left, top, right, bottom = r
        writer.add_bnd_box(int(left), int(top), int(right), int(bottom), label, difficult)
    save_xml_path = 'name.xml'
    ...
    def add_bnd_box(self, x_min, y_min, x_max, y_max, name, difficult=0):
...
class PascalVocReader:
    ...
    def parse_xml(self):
        ...
        w, h, c = size_tag.find('width').text,  size_tag.find('height').text,  size_tag.find('depth').text
        self._image_shape = (int(h), int(w), int(c))
